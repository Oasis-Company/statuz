/**
 * SynRequestIO — Read, write, and validate SYN request YAML files.
 */
...
import type { SynRequest, ValidationResult } from "./types.js";

export class SynRequestIO {
  static read(filePath: string): SynRequest {
    ...
    try {
      ...
      return YAML.parse(raw) as SynRequest;
    } catch (err) {
  ...
  static write(filePath: string, data: SynRequest): void {
    const fullPath = resolve(process.cwd(), filePath);
  ...
  static validate(data: unknown): ValidationResult {
    const schema = SynRequestIO.loadSchema();
  ...
  static validateFile(filePath: string): ValidationResult {
    ...
    try {
      const raw = readFileSync(fullPath, "utf8");
      return SynRequestIO.validate(YAML.parse(raw));
    } catch (err) {
      if (err instanceof YAML.YAMLError) {
        return {
          valid: false,
